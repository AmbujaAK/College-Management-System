	...
	<div class="box-header" style="height:65px">
		...
		<div class="col-xs-12" style="padding-top:10px; padding-bottom:10px">
			<div class="input-group input-group-sm" style="float:left; margin-left:-20px;">
				<a href="dept_add.php"> <button type="button" class="btn btn-block btn-success" style="width:200px">New Department</button></a>
			</div>
		</div>
	</div>
					...
					if(mysqli_num_rows($res_set) > 0){
						while($row = mysqli_fetch_assoc($res_set)){
							echo '<tr>
								...
								<td>'.$row['dept_name'].'</td>
								<td>
									<form method="get" action="dept_edit.php">
										<input type="hidden" name="id" value="'.$row['id'].'">
										<input class="btn btn-primary" type="submit" value="edit">
									</form>
								</td>
								<td>
									<form method="get" action="dept_remove.php">
										<input type="hidden" name="id" value="'.$row['id'].'">
